...
for car_id in np.unique(results['car_id']):
    ...
    x1, y1, x2, y2 = ast.literal_eval(
        results[(results['car_id'] == car_id) & (results['license_number_score'] == max_)]['license_plate_bbox'].iloc[
...
while ret:
    ...
    if ret:
        ...
        for row_indx in range(len(df_)):
            ...
            cy1 = np.abs(int(y1) - int((y2 - y1) * 0.8))
            cy2 = np.abs(int(y1) + int((y2 - y1) * 0.3))
            cx1 = np.abs(int(x1 - (x2 - x1) * 0.2))
            cx2 = np.abs(int(x2 + (x2 - x1) * 0.2))

            frame[cy1:cy2, cx1:cx2, :] = (255, 255, 255)
            cv2.rectangle(frame, (int(cx1), int(cy1)), (int(cx2), int(cy2)), (255, 0, 0), 2)
            
            font_size = min((cy2 - cy1) // 2, (cx2 - cx1) // len(text))
            
            cv2.putText(frame, text, (int(cx1 + 10), int(cy1 + (cy2 - cy1) // 1.5)),
            cv2.FONT_HERSHEY_DUPLEX, font_size / 25, (0, 0, 0), 1, cv2.LINE_AA)
